...
public class QueryIdInfoBusiSVImpl implements IQueryIdInfoBusiSV {
    ...
    public List<BlAcctInfoInfo> queryAcctIdByExtCustId(ExtCustIdInfo extCustIdInfo) {
        ...
        if (CollectionUtil.isEmpty(blCustinfoInfos)) {
            ...
            blAcctInfoInfos = new ArrayList<>();
            for(BlCustinfoInfo custinfoInfo:blCustinfoInfos){
                OwnerIDInfo ownerIDInfo = new OwnerIDInfo();
                ownerIDInfo.setTenantId(extCustIdInfo.getTenantId());
                ownerIDInfo.setOwnerId(custinfoInfo.getCustId());
                List<BlAcctInfoInfo> acctInfoInfos = iQueryIdInfoAtomSV.queryBlAcctInfo(ownerIDInfo);
                if(!CollectionUtil.isEmpty(acctInfoInfos)){
                	blAcctInfoInfos.addAll(acctInfoInfos);
                }
            }
        }
