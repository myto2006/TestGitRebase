...
public class FailedBillMaintainBusiImpl implements IFailedBillMaintainBusi {
    ...
    public List<FailedBill> queryFailedBills(FailedBillCriteria criteria) throws IOException {
        ...
        //时间段的查询
        if(!StringUtil.isBlank(criteria.getStartTime())&&!StringUtil.isBlank(criteria.getEndTime())){//都不为空
        	  Filter filter = new SingleColumnValueFilter("failure_bill".getBytes(), "arrival_time".getBytes(),
                      CompareFilter.CompareOp.GREATER_OR_EQUAL, new BinaryComparator(criteria.getStartTime().getBytes()));
              filterList.addFilter(filter);
              Filter filter1 = new SingleColumnValueFilter("failure_bill".getBytes(), "arrival_time".getBytes(),
                      CompareFilter.CompareOp.LESS_OR_EQUAL, new BinaryComparator(criteria.getEndTime().getBytes()));
              filterList.addFilter(filter1);
        }else if(!StringUtil.isBlank(criteria.getStartTime())&&StringUtil.isBlank(criteria.getEndTime())){//开始时间不为空，结束时间为空
        	Filter filter = new SingleColumnValueFilter("failure_bill".getBytes(), "arrival_time".getBytes(),
                    CompareFilter.CompareOp.GREATER_OR_EQUAL, new BinaryComparator(criteria.getStartTime().getBytes()));
            filterList.addFilter(filter);
        }else if(StringUtil.isBlank(criteria.getStartTime())&&!StringUtil.isBlank(criteria.getEndTime())){//开始时间为空，结束时间不为空
        	 Filter filter = new SingleColumnValueFilter("failure_bill".getBytes(), "arrival_time".getBytes(),
                     CompareFilter.CompareOp.LESS_OR_EQUAL, new BinaryComparator(criteria.getEndTime().getBytes()));
             filterList.addFilter(filter);
        }
        ...
        Iterator<Result> resultIterator = Arrays.asList(resultScanner.next(1000)).iterator();
    ...
    private void fillFailPacketValue(FailedBill failedBill, Result result) {
        ...
        if (cell != null) {
            ...
            try{
            	...
            	failedBill.setFailPacket(new HashMap<String,String>());
            	logger.error(e);
            }
